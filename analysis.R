...
log_attendees_count <- log(attendee_counts)
log_attendee_count_boxplot <- boxplot(log_attendees_count)
...
protest_locations <- protests_df$Location
unique_locations <- length(unique(protest_locations))
...
# `wa_protests_count`.
wa_protests_count <- length(protest_locations[str_detect(protest_locations, "WA")])

# What percentage of all protests occurred in Washington State? Store the result
# in `wa_protest_percentage`
total_protest_count <- length(protest_locations)
wa_protest_percentage <- (wa_protests_count / total_protest_count) * 100
...
count_in_location <- function(location_name, locations_vector) {
  location_count <- length(locations_vector[str_detect(locations_vector, location_name)])
  location_count
}
...
dc_count <- count_in_location("Washington, DC", protest_locations)
...
minneapolis_count <- count_in_location("Minneapolis", protest_locations)
paste("There were", minneapolis_count, "protests in Minneapolis")
...
all_states <- str_sub(protest_locations, -2)
...
states_counts <- sapply(unique_states, count_in_location, protest_locations)
...
# in `max_state_count`
max_state_count <- max(states_counts)
...
dates_in_2020 <- protests_dates[protests_dates >= "2020-01-01" & protests_dates < "2021-01-01"]
...
# filter conditions!
dates_in_2019 <- protests_dates[protests_dates >= "2019-01-01" & protests_dates < "2020-01-01"]
...
describe_protests_on_date <- function(protests_day, date_vector) {
  num_protests_on_date <- length(date_vector[date_vector == protests_day])
  paste("There were", num_protests_on_date, "protests on", protests_day)
}

# Using your function, how many protests were there on May 24th, 2020? Save 
# your result as `num_protests_may_24`
num_protests_may_24 <- describe_protests_on_date('2020-05-24', protests_dates)
...
num_protests_may_31 <- describe_protests_on_date('2020-05-31', protests_dates)
...
protest_counts_by_month <- months(dates_in_2020)
protest_counts_by_month_table <- table(protest_counts_by_month)
...
protest_counts_by_month_2019 <- months(dates_in_2019)
july_2019_protests <- length(protest_counts_by_month_2019[protest_counts_by_month_2019 == "July"])
july_2020_protests <- length(protest_counts_by_month[protest_counts_by_month == 'July'])
change_july_count <- july_2020_protests - july_2019_protests
...
# found/referenced this example of gsub() on stackoverflow: 
# https://stackoverflow.com/questions/29098801/removing-punctuations-from-text-using-r
# googled/read a ton of regex documentation, this being one of them:
